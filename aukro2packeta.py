...
typ, data = imap.search("utf-8", 'UNSEEN SUBJECT "dopravy a platby"')
...
for num in data[0].split():
    ...
    tgt_phone = re.search('%s(.*?)%s' % ('Telefon:', 'Pokud pot'), textCore).group(1).strip()
...
print(_('XML for Packeta generated.'))
if verbose: print("="*70+PacketaPackReqXML+"="*70)
if log: open(log+'/'+str(number)+'_rqst.xml', "w").write(PacketaPackReqXML)

# sending get request and saving the response as response object
resp = requests.get(url = 'https://www.zasilkovna.cz/api/rest', headers = {"Content-Type": "text/xml"}, data = PacketaPackReqXML.encode('utf-8'))
if verbose: print("="*70+'\n'+resp.text+"="*70)
if log: open(log+'/'+str(number)+'_resp.xml', "w").write(resp.text)
